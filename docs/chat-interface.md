...
侧边栏状态管理：
- 应用首次启动时侧边栏默认关闭
- 页面路由变化时自动关闭侧边栏，避免页面切换后侧边栏保持打开状态
- 登录成功后强制关闭侧边栏，确保进入主页面时侧边栏处于关闭状态
- 使用`useState`、`useRef`和自定义事件结合的方式，实现不同组件之间侧边栏状态的协调
...
菜单交互优化：
- 用户菜单打开时防止侧边栏自动关闭，通过自定义事件机制实现
- 菜单箭头添加旋转动画，提供更好的视觉反馈
- 使用`preventCloseRef`引用变量管理关闭状态，确保交互稳定可靠
...
为解决不同组件间状态协调问题，实现了自定义事件通信机制：
- 使用`window.dispatchEvent`和`window.addEventListener`创建自定义事件通道
- 菜单打开状态通过`sidebar-control`事件传递给父组件
- 使用`useRef`引用变量跟踪状态，避免不必要的重渲染
- 组件卸载时清理事件监听器，防止内存泄漏
